...
<!--          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['system:jcSelect:edit']">修改</el-button>
          ...
          <el-button size="mini" type="text" icon="el-icon-upload el-icon--right" @click="handleAddCg(scope.row)" v-hasPermi="['system:jcCg:add']">加入采购单</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="采购对话框" :visible.sync="openAuditStatus" width="700px" append-to-body
               :close-on-click-modal="false">
      <el-form ref="auditForm" :model="auditForm" label-width="110px">
        <el-form-item label="数量" prop="count">
          <el-input v-model="auditForm.count" placeholder="请输入数量" />
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="submitCg">确 定</el-button>
        <el-button @click="cancel">取 消</el-button>
      </div>
    </el-dialog>
  </div>
...
<script>
import { listJcSelect, getJcSelect, delJcSelect, addJcSelect, updateJcSelect, cg } from "@/api/system/jcCg";
...
export default {
  ...
  methods: {
    ...
    submitCg() {
      cg(this.auditForm).then(response => {
        this.$modal.msgSuccess("修改成功");
        this.cancel();
        this.getList();
      });
